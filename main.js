...
const WorkOrderManager = {
    ...
    loadWorkOrderAssets: async () => {
        ...
        if (!supabaseClient) {
            applyAssets([
                { id: 'MRI-001', label: 'MRI-001 - MRI Scanner' },
                { id: 'CT-002', label: 'CT-002 - CT Scanner' },
                { id: 'VENT-003', label: 'VENT-003 - Ventilator' },
                { id: 'MON-004', label: 'MON-004 - Patient Monitor' }
            ]);
            showToast('Database unavailable, using demo assets.', 'warning');
            return;
        }
        ...
        if (error) {
            ...
            showToast('Unable to load assets, using demo list.', 'warning');
            return;
        }
